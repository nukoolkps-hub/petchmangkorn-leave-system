...
**ฝั่งพนักงาน**
- หน้าแรก — สรุปการลารอบนี้ + ยอดสุทธิ (ค่าหัก/โบนัส) · สถิติลากิจ/ลาป่วย · ปฏิทินทีม
...
|---|---|---|---|
| จันทร์–ศุกร์ | เปิด | นับทุกวัน | **300 บาท/วัน** ตั้งแต่วันแรก |
| เสาร์ | **ปิด** | ไม่นับ | — |
| เสาร์ ∈ `extraOpenSaturdays` | เปิด | นับเหมือนวันธรรมดา | เหมือนวันธรรมดา |
| อาทิตย์ | เปิด | นับแยก (คนละอัตรา) | **500 บาท/วัน** |
| อาทิตย์ ∈ `extraClosedSundays` | ปิด | ไม่นับ | — |
| จ–ศ ∈ `extraClosedWeekdays` | ปิด | ไม่นับ | — |

**ไม่มีวันลาฟรี** — ลาวันแรกก็ถูกหัก · ตอนยื่นลาที่มีวันอาทิตย์
ระบบบังคับให้ติ๊กรับทราบก่อนถึงจะกดส่งได้
...
| เงื่อนไข | ผล |
|---|---|
| **ไม่ลาวันธรรมดาเลย** ทั้งรอบ | ได้ **+300 บาท** (ลาวันอาทิตย์ไม่ทำให้เสียก้อนนี้) |
...
แก้อัตราหัก/โบนัสได้ที่ `src/constants.ts` → `BUSINESS_RULES`
(`WEEKDAY_LEAVE_DEDUCTION` · `SUNDAY_LEAVE_DEDUCTION` ·
`NO_WEEKDAY_LEAVE_BONUS` · `PERFECT_ATTENDANCE_TOPUP`)
...
├── utils/leaveUtils.ts          นับวันลา/ค่าหัก/โบนัส (มี unit test)
