...
**ฝั่งพนักงาน**
- หน้าแรก — สรุปการลารอบนี้ + ยอดสุทธิ (โควต้า/ค่าหัก/โบนัส) · สถิติลากิจ/ลาป่วย · ปฏิทินทีม
...
| การลา | สรุปลา | ใครลาไปกี่วัน — รายคนในรอบ + สรุปรายปี |
| การลา | สรุปรอบจ่าย | รอบนี้จ่ายเท่าไหร่ — ตารางหัก/โบนัส/สุทธิทุกคน + ปุ่มคัดลอก · ปิด-เปิดรอบ · ล็อกยอด |
...
|---|---|---|---|
| จันทร์–ศุกร์ | เปิด | นับเข้าโควต้า **1 วัน/รอบ** | เกินโควต้า **300 บาท/วัน** |
| เสาร์ | **ปิด** | ไม่นับ | — |
| เสาร์ ∈ `extraOpenSaturdays` | เปิด | นับเหมือนวันธรรมดา | เหมือนวันธรรมดา |
| อาทิตย์ | เปิด | นับแยก — ไม่กินโควต้าวันธรรมดา | **500 บาท/วัน ตั้งแต่วันแรก** |
| อาทิตย์ ∈ `extraClosedSundays` | ปิด | ไม่นับ | — |
| จ–ศ ∈ `extraClosedWeekdays` | ปิด | ไม่นับ | — |

**โควต้า = ไม่ถูกหักเงิน ไม่ใช่ "ลาฟรี"** — ลาวันธรรมดาแม้อยู่ในโควต้าก็
**เสียโบนัสทั้ง 1,000 ทันที** วันแรกจึงแพงที่สุด ไม่ใช่ถูกที่สุด ·
ตอนยื่นลาที่มีวันอาทิตย์ ระบบบังคับให้ติ๊กรับทราบก่อนถึงจะกดส่งได้

### รอบจ่ายเงินเดือน

ถ้าคิดเงินเดือนก่อนสิ้นเดือน (เช่นวันที่ 27) กด **"ปิดรอบ"** ที่หน้า **สรุปรอบจ่าย**
...
ค่าหักและโบนัสผูกกับ **รอบ** ไม่ใช่เดือนปฏิทิน — พนักงานเห็นช่วงวันของรอบ
บนการ์ดสรุปหน้าแรก จะได้ตรงกับที่ ADMIN จ่ายจริง

### ล็อกยอด — หลังเที่ยงคืนของวันที่กดปิดรอบ

กด "ปิดรอบ" แล้วยอด **ยังไม่ล็อกทันที** — วันที่กดยังแก้ใบลา/ปฏิทินร้านได้อยู่
(เผื่อลงใบลาไม่ครบ) · พ้นเที่ยงคืนไปแล้วยอดถึงจะล็อกตามที่เป็นตอนสิ้นวันนั้น

```
กดปิดรอบ 27 ส.ค.  →  27 ส.ค. ยังแก้ได้ (ตารางโชว์ยอดสด)
                   →  28 ส.ค. เป็นต้นไป: ล็อก
```

ล็อกแล้วจะแก้ปฏิทินร้านหรือใบลาย้อนหลังยังไง ยอดที่จ่ายไปแล้วก็ไม่ขยับ ·
ถ้ายอดสดต่างจากที่ล็อก ตารางสรุปจะขึ้นแถบเตือนบอกว่าใครขยับจากเท่าไหร่
เป็นเท่าไหร่ พร้อมปุ่ม "ยึดยอดใหม่" ให้ ADMIN ตัดสินใจ

### โบนัส 2 ก้อน (คิดจากทั้งรอบ)

| เงื่อนไข | ผล |
|---|---|
| **ไม่ลาวันธรรมดาเลย** ทั้งรอบ | ได้ **+300 บาท** (ลาวันอาทิตย์ไม่ทำให้เสียก้อนนี้ · แต่ลาวันธรรมดาแม้ในโควต้าก็เสีย) |
| **ไม่ลาเลย** ทั้งรอบ | ได้ top up อีก **+700** → รวม **+1,000 บาท** |

ก้อนที่สองทับบนก้อนแรก ไม่ใช่แทนที่ · ผลที่ตามมาจากสูตรนี้:
...
| ลาวันธรรมดา 1 วัน (ในโควต้า) | ไม่ถูกหัก แต่ไม่ได้โบนัส = **0** |
| ลาวันธรรมดา 2 วัน | **−300** |
| ลาวันธรรมดา 1 + อาทิตย์ 1 | **−500** |

ตอนยื่นใบแรกของรอบ ระบบจะบอกเลยว่า "ใบนี้ทำให้เสียโบนัสเท่าไหร่" ไม่ใช่แค่
ค่าหัก · ลาวันที่ร้านปิดไม่ทำให้เสียโบนัส
...
(`WEEKDAY_LEAVE_QUOTA` · `OVER_QUOTA_WEEKDAY_DEDUCTION` ·
`SUNDAY_LEAVE_DEDUCTION` · `NO_WEEKDAY_LEAVE_BONUS` ·
...
├── utils/leaveUtils.ts          นับวันลา/โควต้า/ค่าหัก/โบนัส (มี unit test)
├── utils/periodSettlement.ts    สรุปเงินทั้งรอบ + snapshot ล็อกยอด (มี unit test)
