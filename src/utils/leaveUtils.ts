 ...
 *  countWeekdayLeaves/getDeductibleLeaveDays เพื่อ clamp ให้แต่ละรอบนับเฉพาะวัน
 *  ของตัวเอง */
...
/** จำนวนวันลาที่ "ถูกหัก" แยกตามอัตรา — ทุกวันที่ร้านเปิดถูกหักหมด
 *  IMPORTANT: นับเป็น "วัน" ไม่ใช่ "ใบลา" · ใบเดียวยาว 3 วัน = 3 วัน */
export function getDeductibleLeaveDays(
  monthLeaves: { start: string; end: string }[],
  calendar?: StoreCalendar | null,
  period?: PeriodArg,
...
/* ─── ค่าหักเงินจากการลา ─────────────────────────────────────────
   ...
   - วันธรรมดาที่ลา → หักวันละ WEEKDAY_LEAVE_DEDUCTION ทุกวัน (ไม่มีวันฟรี)
   - วันอาทิตย์ที่ร้านเปิด → หักวันละ SUNDAY_LEAVE_DEDUCTION
...
export interface LeaveDeduction {
  /** จำนวนวันธรรมดาที่ลา (ร้านเปิด) */
  weekdayDays: number;
  /** จำนวนวันอาทิตย์ที่ลา (ร้านเปิด) */
  sundayDays: number;
  /** ยอดหักจากวันธรรมดา (บาท) */
 ...
 *  ให้แต่ละรอบคิดเฉพาะวันของตัวเอง (โบนัสเป็นรายรอบ) */
export function getLeaveDeduction(
  monthLeaves: { start: string; end: string }[],
  calendar?: StoreCalendar | null,
  period?: PeriodArg,
): LeaveDeduction {
  const { weekdays, sundays } = getDeductibleLeaveDays(
    monthLeaves,
    calendar,
    period,
  );
  const weekdayAmount = weekdays * WEEKDAY_LEAVE_DEDUCTION;
 ...
 *  ไม่ใช่คิดใบใหม่ลอย ๆ — ใบใหม่อาจทับวันกับใบเดิม ซึ่งไม่ควรถูกหักซ้ำ */
